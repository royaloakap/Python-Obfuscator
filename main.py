...
class Royal:
    ...
    def __init__(self, content: str, clean = True, obfcontent = True, renlibs = True, renvars = True, addbuiltins = True, randlines = True, shell = True, camouflate = True, safemode = True, ultrasafemode = False) -> None:
        ...
        r"""
        Utilisez les modes sans échec uniquement si vous rencontrez des erreurs avec votre script obf [!!!]
        ## Settings

        clean:Utilisez-le si vous souhaitez nettoyer le script (compresser les intentions, supprimer les commentaires...)
        >>> # [!!!] Attention, cela peut provoquer quelques bugs [!!!]

        obfcontent: Utilisez-le si vous souhaitez masquer le contenu des variables
        >>> # [!!!] Attention, cela peut provoquer quelques bugs on very complex scripts, but generally it should work [!!!]

        renlibs: Utilisez-le si vous souhaitez redéfinir les bibliothèques
        >>> # [!!!] Attention, cela peut provoquer quelques bugs on very complex scripts, but generally it should work [!!!]

        renvars: Utilisez-le si vous souhaitez redéfinir les variables
        >>> # [!!!] Attention, cela peut provoquer quelques bugs on very complex scripts, but generally it should work [!!!]

        addbuiltins:Utilisez-le pour renommer également les éléments intégrés uniquement si vous n'avez pas utilisé les mêmes variables
        >>> # [!!!] Attention, cela peut provoquer quelques bugs [!!!]

        randlines: Utilisez-le uniquement si vous n'avez pas défini de variable sur plusieurs lignes!
        >>> # [!!!] Attention, cela peut provoquer quelques bugs [!!!]

        shell: Utilisez-le pour ajouter un shell à chaque morceau de votre script
        >>> # [!!!] Attention, cela peut provoquer quelques bugs [!!!]

        camouflate:Utilisez-le pour camoufler le script final
        >>> # [!!!] No bugs [!!!]


        safemode: Utilisez ceci si vous avez utilisé des arguments positionnels/arguments prédéfinis dans vos fonctions
        >>> # [!!!] No bugs [!!!]

        ultrasafemode: Utilisez ceci pour ignorer les couches les plus susceptibles de provoquer des erreurs
        >>> # [!!!] No bugs [!!!]
        """
        ...
        if obfcontent:
            p("Obfuscating le contenu de chaque variable...")
            self.ObfContent()

        if clean:
            p("Clean le script basic !")
            self.CleanCode()

        if not self._verify_lin(content):
            print(stage("Supports multi-lignes détectés ! Sauter les couches protégeant les morceaux.", '!!!', Col.light_red, Col.light_red))
            randlines, shell = False, False

        if randlines:
            p("Ajouter des lignes aléatoires entre chaque morceau...")
            self.RandLines()

        if shell:
            p("Ajouter un shell à chaque morceau...")
            self.Shell()
        ...
        if clean:
            p("Nettoyer le script final!")
            self.CleanCode()

        p("Compression du script final...")
        self.Compress()

        if camouflate:
            p("Camoufler le script final pour le rendre moins suspect...")
            self.Camouflate()
        else:
    ...
    def AntiSkid(self):
        if self.camouflate:
            self.content = fr"""
# GG! Vous venez de désobscurcir un fichier obscurci avec Royal

# Congratulations!

# https://t.me/Royaloakap

# by Royaloakap and Royal


try:
    if (
        __obfuscator__ != "Royal" or
        __authors__ != ("Royaloakap", "Royal") or
        __github__ != "https://t.me/Royaloakap" or
        __discord__ != "https://discord.gg/RoyalC2" or
        __license__ != "EPL-2.0" or
        __code__ != 'print("Rejoins le meilleur server !")'
    ):
        int('skid')
except:
    input("Les roses sont rouges\Les violettes sont bleues\Tu es un dérapage\personne ne t'aime")
    __import__('sys').exit()
...
{0}()['{3}']={2}('builtins').vars"""[1:].format(self.globals, self.locals, self.__import__, self.vars, self.unhexlify).splitlines()
    ...
    def RenameImports(self):
        ...
        if _imports == False:
            print(stage("Importation d'étoiles détectée ! Ignorer le renommage des bibliothèques et variables importées.", '!!!', Col.light_red, Col.light_red))
...
__obfuscator__ = 'Royal'
__authors__ = ('Royaloakap', 'Royal')
__github__ = 'https://t.me/Royaloakap'
__discord__ = 'https://discord.gg/RoyalC2'
__license__ = 'EPL-2.0'

__code__ = 'print("Rejoins le meilleur server discord.gg/RoyalC2 !")'
...
""".strip()
    ...
    class StarImport(Exception):
        def __init__(self):
            super().__init__("Star Import est interdit, veuillez mettre à jour votre script")
    ...
    def _randvar2(self):
        return ''.join(choice('billythegoat356Royal') for _ in range(randint(5, 20)))
...
text = r"""
  ...
  _____                   _             _               
 |  __ \                 | |           | |              
 | |__) |___  _   _  __ _| | ___   __ _| | ____ _ _ __  
 |  _  // _ \| | | |/ _` | |/ _ \ / _` | |/ / _` | '_ \ 
 | | \ \ (_) | |_| | (_| | | (_) | (_| |   < (_| | |_) |
 |_|  \_\___/ \__, |\__,_|_|\___/ \__,_|_|\_\__,_| .__/ 
               __/ |                             | |    
              |___/                              |_|         
"""[:-1]
...
def main():
    ...
    print('\n')
    file = input(stage(f"Faites glisser le fichier que vous souhaitez obf {dark}-> {Col.reset}", "?", col2 = bpurple)).replace('"','').replace("'","")
    print('\n')
    ...
    skiprenaming = input(stage(f"Ignorer le renommage des bibliothèques et des variables {dark}[{light}y{dark}/{light}n{dark}] -> {Col.reset}", "?")).replace('"','').replace("'","") == 'y'
    print()
    skipchunks = input(stage(f"Skip the protection of chunks {dark}[{light}y{dark}/{light}n{dark}] -> {Col.reset}", "?")).replace('"','').replace("'","") == 'y'
    camouflate = input(stage(f"Camoufler le code final pour le rendre moins suspect {dark}[{light}y{dark}/{light}n{dark}] {Col.reset}", "?")).replace('"','').replace("'","") == 'y'
    ...
    now = time()
    Hype = Royal(content=script, renvars = renvars, renlibs = renlibs, randlines = randlines, shell = shell)
    script = Hype.content
    ...
    print('\n')
    getpass(stage(f"Obfuscation terminée avec succès dans {light}{now}s{bpurple}.{Col.reset}", "?", col2 = bpurple))
if __name__ == '__main__':
    main()